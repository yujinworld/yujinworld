<div align="center">

# 안녕하세요, 김유진입니다 👋

**비즈니스와 데이터를 잇는 데이터 분석가**

데이터를 통해 비즈니스를 이해하고 성장 기회를 발굴합니다.
다양한 기법으로 인사이트를 도출하고, 누구나 이해할 수 있도록 전달하는 것을 중요하게 생각합니다.

[![GitHub followers](https://img.shields.io/github/followers/YOUR_USERNAME?label=Follow&style=flat-square&color=4A90D9)](https://github.com/YOUR_USERNAME)
[![Profile views](https://komarev.com/ghpvc/?username=YOUR_USERNAME&style=flat-square&color=4A90D9)](https://github.com/YOUR_USERNAME)

</div>

---

## 소개

- 하나은행 **Digital Hana 路 데이터 분석·서비스 기획 4기** 기수 1등(97.76점) · 최우수 수료
- **메가존클라우드** AIR Strategy · AI/ML Engineer 인턴십 수료 (2025.11 ~ 2026.01)
- 한양대학교 프로그래밍 동아리 **FORIF 회장** 역임 — Python·SQL·Tableau 멘토
- 비전공자 출신으로 전국 단위 공모전 2회 수상 및 KSC 학부생 논문 게재

---

## 기술 스택

**데이터 분석 · 시각화**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white)

**데이터베이스 · 쿼리**

![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)

**클라우드 · 기타**

![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazon-aws&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

---

## 주요 프로젝트 · 수상

**날씨 빅데이터 콘테스트 안전·방재 분야 장려상** `2025.08`
기상·재난 데이터와 고령자 인구 데이터를 결합한 XGBoost 기반 일일 신고건수 예측 모델 개발 및 대시보드 구현

**춘천시 생성형 AI 아이디어 경진대회 3위 장려상** `2025.06`
청년 창업 지원 특화 플랫폼 Blueming — 4개 전문 에이전트 기반 설계 · 프로토타입 시연

**KSC 학부생 논문 poster session 게재** `2025.12`
저성능 LLM 앙상블 vs 단일 고성능 LLM 주가 예측 정확성 비교 (제2저자)

---

## 자격증

![ADsP](https://img.shields.io/badge/ADsP-데이터분석준전문가-0066CC?style=flat-square)
![빅데이터분석기사](https://img.shields.io/badge/빅데이터분석기사-한국데이터산업진흥원-0066CC?style=flat-square)

---

## GitHub 통계

<div align="center">

<img height="160" src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=default&hide_border=true&count_private=true&include_all_commits=true&locale=kr" />
&nbsp;&nbsp;
<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=default&hide_border=true&langs_count=6" />

</div>

---

<div align="center">

한양대학교 경제금융학부 졸업예정 (2026.02) · kujin920@gmail.com

</div>
